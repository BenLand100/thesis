...
Tests performed to ensure the source was properly fabricated and meets SNO+ requirements for cleanliness are presented in \Cref{chap:tests}.
...
The decay chamber is a sphere made out of UVA acrylic supplied by RPT (Reynolds Polymer Technology). 
...
To handle the different types of connections used in different phases of SNO+, a flange was added to the original steel housing, as shown in \Cref{fig:flange}. 
...
The SNO \Li umbilical was re-used to transport \Li into the source during the SNO+ water phase.
...
The so-called ``lighthouse" attachment (\Cref{fig:connection}) is attached to the top of the source and is used to attach the source to SNO+'s source deployment hardware.
...
By generating stereolithography (STL) surface definitions, which break an arbitrary surface down into triangles, and importing these into GEANT4, it was possible to simulate how paint roughness or anisotropy would change the peak NHits. 
\Cref{fig:stltests} summarizes the result that the expected roughness and anisotropy had no significant effect on the mean NHit. 
...
Many options for directly measuring the paint layer thickness were explored and ultimately rejected based on cost or lack of required precision. The final decision was to infer the paint thickness from the change in mass of the source before and after painting each layer. With a total source weight of 8.2\,kg and an expected total paint weight of 50\,g this required a scale capable of weighing 0.1\,g on top of 10\,kg so that uncertainty in weight was not a dominant source of error. The densities of the dry paint used in the black and white layers were fit using volume and mass measurements of small dry samples. These density results are summarized in \Cref{fig:paintdensity}, and errors on these densities were the dominate sources of error for the thickness calculation. 
...
Estimates for the radon emanation of the SNO+ calibration hardware used a measurement of $4.60$~$\mu$Bq~m$^{-2}$ for stainless steel \cite{kormos:2015}. 
...
This will allow deployment of the Cherenkov source as a calibration for the absolute light collection in SNO+ in future phases
...
In addition, the PMT signal was connected to the trigger signal digitizer (CAEN) in the SNO+ trigger system to record the full pulse.
...
The distribution of number of hit PMTs in the SNO+ detector for each of these classifications is shown in \Cref{fig:chsrc_nhits} while \Cref{fig:chsrc_pmttraces} shows the average digitized Cherenkov source tag PMT signal for each class.
...
\caption{\label{fig:chsrc_nhits} The hit PMT distributions in the SNO+ detector for the different classifications of Cherenkov source data.}
\end{figure}
...
\includegraphics[width=.92\textwidth]{chsrc_nhit}
\caption{Distribution of number of Cherenkov photons exiting the source for events without scintillation light.}
\label{fig:nphotons}
\end{subfigure}
\hspace{0.5cm}
\begin{subfigure}{.48\textwidth}
\includegraphics[width=.92\textwidth]{chsrc_fit}
\caption{Fit of NHit-distribution to sum of binomial distributions.}
\label{fig:fit}
\end{subfigure}
\caption{Using GEANT4~\cite{geant4} based simulations of the Cherenkov source to extract the overall light collection efficiency in SNO+. Figures taken from~\cite{Heintzelman:2013}. }
