...
A schematic of CHESS is shown in \Cref{fig:timing-setup,fig:timing-setup-2}.  
...
The PMTs are held in a 7x7 grid, with four additional slots on each side, as shown in~\Cref{fig:timing-setup}. 
...
Two custom-made cylindrical scintillator tags are positioned above and below the target vessel (\Cref{fig:timing-setup}) in order to trigger on vertical cosmic muons.  
...
\section{The DAQ System \label{sec:daq}}
...
Primary particles are produced (\Cref{sec:primarygen}) and tracked in a complete geometry implemented in GEANT4~\cite{geant4}. 
When Cherenkov and scintillation processes take place (\Cref{sec:photon_prod}), individual photons are tracked from the production point to the PMTs (\Cref{sec:optics}). 
Once the photon reaches the PMT, a custom model decides whether to produce a photoelectron (PE) based on the photon properties and PMT features (\Cref{sec:pmt_model}). 
If any PE are produced in a PMT for an event, a custom DAQ simulation (\Cref{sec:daq_sim}) generates a simulated waveform for that PMT to be analyzed with the same algorithms applied to data.
...
For reference, the spectra for LAB~\cite{lab_emission} and LAB+PPO~\cite{snop_private} are shown in \Cref{fig:optics}. 
...
If the incident photon creates a PE, the total amplified char charge and detection time time are extracted from Gaussian distributions that have been previously calibrated to take into account the PMT gain and electronic delays (\Cref{sec:calibration}).
...
The SPE charge distribution for individual PMTs was measured (\Cref{sec:calibration}), and \Cref{fig:spe_data} shows that the Gaussian model agrees well with the data.
...
These are measured for each cPMT (\Cref{sec:calibration}).
...
This threshold is determined per PMT by modeling the PMT pulse shape as a log-normal pulse with parameters extracted from the fit in \Cref{sec:pmtpulses} and requiring integrated charge equivalent to the mean of the SPE distribution for each PMT (\Cref{pmt-gain}). 
...
A small PMT optically coupled to the target is used to trigger the acquisition (\Cref{sec:triggering}). 
...
The SPE charge distribution for one of the array PMTs  is shown in \Cref{fig:spe_data}, where a clear SPE distribution is seen. 
