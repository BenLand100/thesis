...
The CHESS detector fully digitizes the PMT signals for each triggered events.
...
Simulations of these events shown in \Cref{f:clipMC} show a clear tail in the PE distribution observed on the inner PMT group for both water and LAB+PPO targets.
...
The hit-time residuals are evaluated as the PMT hit times with respect to the event time, corrected by per-channel delays (Sec.~\ref{s:delay}) and by the photon ToF.  
...
A comparison of using this definition of event time and event time defined with respect to the the muon tag is shown in \Cref{f:smear}, overlaid with the MC prediction in each case.  
...
Prior to moving to LAB+PPO, pure LAB was deployed. 
...
The charge ratio for LAB+PPO is shown in Fig.~\ref{f:labppoQ}. 
...
Shown in this section are timing and charge results for 1\% and 5\% scintillator mixtures that are expected to have roughly 1\% and 5\% of LAB+PPO light yield.
Because the scintillation component is expected to be very dim, the results shown here for charge are directly in units of PE instead of the charge ratio shown for brighter LS in previous sections.
...
The total number of PE distributions shown in \Cref{fig:wbls5pct_totalq} again indicates charge based separation will be difficult, as the middle group of PMTs is only seeing a slightly higher number of PE on average.
