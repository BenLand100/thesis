...
Roughly speaking, this analysis will be searching for a deficite of neutrinos resulting from decay of neutrinos in flight from the Sun to the Earth.
...
The fit will minimize the negative SNO Extended Log Likelihood function~\cite{3phase} using this model for the survival probability.
...
\label{decay}
\end{equation}
The survival probabilities for electron and muon/tau flavor neutrinos in a decaying scinareo may then be calculated with appropriate factors from the PMNS matrix.
\begin{equation}
\begin{array}{rcl}
...
For this analysis the lifetimes of mass state 1 and 3 are fixed at infinity because the $^8$B flux exits the sun as a nearly pure $\nu_2$ mass state, so the data taken by SNO for $^8$B neutrinos is nominally insensitive to $\nu_1$ or $\nu_3$ decay.
...
For this analysis, I have built off the existing QSigEx code which properly handled nuisance parameters, pdf generation, data binning, and likelihood function implementation for the published 3-phase analysis of SNO data \cite{3phase}.
...
The physical observables determined from vertex reconstruction can used to statistically sort events into different classes of interactions.
Here the statistical sorting is be done with a maximum likelihood fit where the number of events in each class of interaction is floated, and each class of event is described by a probability distribution function (PDF) of some observable quantities.
...
\begin{enumerate}
    ...
    \item Radius - the volume weighted radius of the event estimated from reconstruction. Useful for distinguishing backgrounds from intrinsic radioactivity which tend to be away from the center of the detector.
    \item Isotropy - the $\beta_{14}$ parameter based on Legendra polynomials giving a direction independent measure of isotropy of hits. Can distinguish between multi site events (multiple gammas scattering multiple electrons - NC) and single electrons (CC, ES). 
    \item Direction - the cosine of the angle between the event direction and the direction to the Sun at the time of the event. Identifies ES electrons which are highly correlated with the solar direction, and provides some discrimination for CC electrons.
\end{enumerate}
...
A fit package for all three phases of SNO data that treated $P_{ee}$ and $P_{ea}$ independent polynomials existed in the software package QSigEx~\cite{plthesis}.
This package already implemented the SNO Extended Log Likelihood Function, and could be modified to use the model described in \Cref{lifetime_model} instead of polynomials.
...
\subsection{SNO Extended Log Likelihood}

QSigEx finds the model parameters best describing the data by maximizing the Poisson likelihood of the observed data.
...
\end{equation}
With PDFs provided from SNOMAN, and data from the detector, carrying out such a fit is relatively straightforward.
The results relavent to this thesis are obtained by the construction of the PDFs for the solar signals.
...
Unfortunately preliminary tests of the binned fit had very poor results regarding at capturing the shape of the lifetime model survival probabilities.
...
Previous analyses \cite{plthesis} have encountered a similar issues and opted for a strategy of finding a preferred $^8$B flux in some reduced fit and then performing further fits and statistical studies with the flux fixed, and incorporating uncertainty from flux variations as a final step.
...
\begin{enumerate}
\item Perform a scan over $k_2$ minimizing floated parameters at each point. For data fits this can be done along with the other scanned nusiance parameters.
...
Each of the observables has many systematic parameters associated with it representing possible descrepancies between simulated and real data.
Many of these parameters are best constrained with the data itself, and central values along with their uncertanties are determined with a brute-force initial iterative scan of the data.
These scanned parameters, along with the other systematics, were held fixed during the central fit, and their errors were propagated with a shift and refit algorithm.

The initial iterative minimization used here proceeds as follows:
\begin{enumerate}
\item A parameter is scanned around its current best guess
\item An asymmetric gaussian is fit to the scan $-\log{L}$ to determine the central and upper/lower uncertainties.
\item The best guess for that parameter is updated to the fitted values just determined.
...
Other systematic uncertainties are constrained with in-situ and ex-situ measurements.
These scanned parameters, along with the other systematics, were held fixed during the central fit, and their errors were propagated with a shift and refit algorithm:
\begin{enumerate}
    ...
    \item For each set of parameters, the fit is minimized again and the most likely value of each floated parameter is recorded.
    \item The RMS of these shift-and-refit $k_2$ values on each side of the central value for $k_2$ are calculated as the systematic effect. 
    \item The likelihood profile for $k_2$ from the central fit is shifted by the $k_2$ shift-and-refit distribution, and the average likelihood is used to set the final limits
\end{enumerate}

Both positive and negative uncertainties are quoted but it is important to note that this analysis is expected to produce a lowerbound on $k_2$ making the positive error rather meaningless.
...
The parameters considered here include nuisance parameters and all parameters scanned as the preliminary step in the fit. \Cref{fig:detector_systematics1,fig:detector_systematics2} show the individual contributions of the scanned parameters to the detector systematic uncertainty on $k_2$: $^{+13.4\%}_{-9.03\%}$.

\begin{figure}
\centering
\includegraphics[width=0.8\columnwidth]{basic_1_2}
...
\begin{figure}
\centering
\includegraphics[width=0.8\columnwidth]{basic_2_2}
