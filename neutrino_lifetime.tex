...
For this analysis, I have built off the existing QSigEx code~\cite{plthesis}, which properly handled nuisance parameters, PDF generation, data binning, and likelihood function implementation for the published 3-phase analysis of SNO data~\cite{sno}.
...
Here the statistical sorting is done with a maximum likelihood fit: QSigEx finds the model parameters best describing the data by varying these parameters and maximizing the Poisson likelihood, $\mathscr{L}$, of the observed data.
This is done by minimizing the well-known form of the negative log likelihood:
\begin{equation}
\mathcal{L} = -\log{\mathscr{L}} = \nu - \sum_{i=1}^{n_{events}} \log \left[ \nu f(\vec{x_i}) \right],
\end{equation}
...
In the case of many classes of events with Poisson parameters $\nu_j$ described by PDFs $f_j$ this negative log likelihood this can be written as:
\begin{equation}
...
$M$ is chosen from a Poisson distribution with a mean equal to the expected number of events for that event class and phase from previous SNO analyses~\cite{leta,sno}.
...
\subsection{Solar-Signal with High Statistics Backgrounds}
...
\subsection{Solar-Signal with All Backgrounds}
\label{3phase_allbg}
...
Notably these results are comparable to ensemble testing of the published SNO 3-phase results~\cite{sno}.
...
\section{Results from the \texorpdfstring{\nicefrac{1}{3}}{1/3} Dataset}
\label{third}
...
To reduce computational power required for this test, the central values of scanned nuisance parameters from the 3-phase $P_{ee}+P_{ea}$ fit~\cite{plthesis} were used. 
...
Super-Kamiokande~\cite{superk}, KamLAND~\cite{kamland8b} and Borexino~\cite{borexino8b} have all measured the $^8$B neutrino flux via the elastic scatter channel. 
...
\caption{The profiled likelihood for the parameter $k_2$ using $^8$B measurements from SuperK~\cite{superk}, Borexino~\cite{borexino8b}, and KamLAND~\cite{kamland8b}.}
