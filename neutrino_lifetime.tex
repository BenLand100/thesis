...
Relatively recent precise mixing parameters from KamLAND~\cite{kamland} and DayaBay~\cite{dayabay} and improved theoretical predictions for the $^8B$ flux~\cite{serenelli} mitigate this to some extent.
...
\subsection{MSW Effect + Decay Survival Probability}

The flavor basis is related to the mass basis by the PMNS matrix $U_{\alpha i}$ as follows
\begin{equation}
\ket{\nu_{\alpha}} = \sum_i U^*_{\alpha i} \ket{\nu_i}.
\end{equation}
The free evolution of these states is most easily represented in the mass basis where the mass states have a diagonal Hamiltonian
\begin{equation}
...
\end{bmatrix}
\end{equation}
The survival probability for flavor state $\ket{\nu_\alpha}$ to be detected as flavor state $\ket{\nu_\beta}$ at some time later under free evolution for a distance L is therefore
\begin{equation}
P_{\alpha\rightarrow\beta} = \left|\braket{\nu_\beta(t)}{\nu_\alpha}\right|^2 = \left|\sum_i U^*_{\alpha i} U_{\beta i} e^{- i m_i^2 L/(2E)} \right|^2.
\end{equation}
The MSW~\cite{wolfenstein,mikheyev} effect proposes that the coherent forward scatter of electron flavor neutrinos off of electrons in a material adds a potential $V_e$ to electron flavor neutrinos which depends on the local electron density $n_e$
\begin{equation}
V_e = \sqrt{2} G_F n_e.
\end{equation}
Written in the mass basis $H_{MSW}$ then takes the form
\begin{equation}
...
It is useful to introduce the matter mass basis $\ket{\nu_{mi}(V_e)}$ which are eigenstates of the Hamiltonian $H_{MSW}$ at a particular electron potential $V_e$.
Note that if the electron density is zero the evolution is in vacuum so $\ket{\nu_{mi}(0)} \rightarrow \ket{\nu_{i}}$.
...
This adiabatic evolution is realized in the Sun~\cite{3phase} so knowing where in the Sun a neutrino is produced, or more precisely the electron density at the production point, one can calculate the eigenstate composition for as long as the adiabatic condition is satisfied.
...
Further due to the propagation distance between the Earth and the Sun these mass state fluxes can be assumed to be incoherent once they arrive at Earth~\cite{3phase}.
Therefore, the flux $\phi_i$ of a neutrino mass state at earth due to solar neutrinos produced at an electron potential $V_e$ under the MSW effect can be calculated as
\begin{equation}
\phi_i = \left| \braket{\nu_{m i}(V_e)}{\nu_e} \right|^2.
\label{rawflux}
\end{equation}

This flux of a particular mass state $\ket{\nu_i}$ could have some lifetime associated with it $\tau_i$ representing the decay of neutrinos of that mass state.
...
Therefore, the detected flux $\psi_i$ of a neutrino mass state at earth can be given as 
\begin{equation}
