...
Because of its depth, SNO+ has much lower cosmic muon flux than other water Cherenkov detectors, so even though it is much smaller than, for instance, Super-Kamiokande, it is able to make competitive measurements of nucleon decay \cite{nucleon_decay} because of lower cosmogenic backgrounds.
...
This represents the first physics results from the SNO+ detector, and also serves as a cross check that SNO+ is correctly simulating its direction resolution.
...
The SNOMAN package used to model and analyze data from SNO is replaced by the RAT Monte Carlo and analysis package for SNO+.
...
This is demonstrated by generating background PDFs as shown in \Cref{fig:solar:backgrounds} of all backgrounds considered for SNO+.
...
\caption{\label{fig:energy_lookup}The lookup table used in the first step of correcting reconstructed energy in SNO+. This plot shows the residual of the correction, and applies to both real and simulated data.}
\end{figure}
...
E & 6.49e-13 & 1.43e-13
\end{tabular}
\caption{\label{tbl:energy_correction}Coefficients used to further correct the reconstructed energy of SNO+ data according to \Cref{tbl:energy_correction}. This was done separately for real and simulated data after the lookup table in \Cref{fig:energy_lookup} was applied.}
\end{table}
...
As the solar MC is simulated as a uniform distribution inside the AV, and the position resolution of SNO+ is small (O(10cm)), smearing the reconstructed position by this resolution has a 
...
The SNO+ date for the runs included along with the 5 selected events is shown in \Cref{fig:solar:opendata} to gain confidence that these events are not suspiciously distributed in time.
...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{snoplus_solar_binned}
\caption{The total number of ES events from each energy bin in the SNO+ $^8$B ES analysis is shown in the top panel. 
...
\caption{The best fit for the SNO+ $^8$B ES analysis, shown with data from the full 5.0-15.0~MeV energy range.}
...
\caption{The fit results from the SNO+ $^8$B ES analysis excluding the lowest energy bin from 5.0-6.0~MeV. Notably, this demonstrates SNO+ has very low backgrounds above 6.0~MeV.}
...
If this bin is excluded, \Cref{fig:solar:lowbg} shows very low backgrounds SNO+ achieved above 6.0~MeV.
...
This demonstrates that SNO+ has achieved very low backgrounds above 6.0~MeV, which is an excellent conclusion to arrive at as SNO+ moves into scintillator phase.
